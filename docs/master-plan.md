...
Migration hygiene: **done.** The schema is no longer `db push`ed — three committed migrations
(`init`, `fix_natural_keys`, `search_indexes`) reproduce the database deterministically on every
machine and sibling project.

## 5. Execution phases

_Phases 1–3 and the entire Pokedex product plan (R1–R5) are complete; they are kept below as the
record of what was built and why. Phase 4 (School) is the active frontier._

**Phase 1 — Database up (unblocks everything)** ✅ **DONE**
...
**Phase 2 — GraphQL API (the heart)** ✅ **DONE** — `apps/pokedex-service` serves 20 read-only queries
...
**Phase 3 — Typed consumption** ✅ **DONE** — `libs/data-access-pokedex` + `gqlResource`/`httpResource`
...
**Pokedex product releases — R1–R5** ✅ **DONE** — game context, findability, depth, intelligence
